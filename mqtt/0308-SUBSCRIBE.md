...
客户端向服务端发送SUBSCRIBE报文用于创建一个或多个订阅。每个订阅注册客户端关心的一个或多个主题。为了转发应用消息（它们以与订阅相匹配的主题发布），服务端发送PUBLISH报文给客户端。SUBSCRIBE报文也（为每个订阅）指定了最大的QoS等级，服务端根据这个发送应用消息给客户端。
...
请求的最大服务质量等级字段编码为一个字节，它跟在以UTF-8编码的主题名后面，那些主题过滤器 /和QoS等级组合是连续地打包。
